...
class DialogueTemplate:
    ...
    def get_inference_prompt(self) -> str:
        if self.dialogue_type == "default":
            ...
            return prompt
        elif self.dialogue_type == "codellama":
            prompt = (
                self.user_token + "\n" + (self.system + "\n" if self.system else "")
            )
            for message in self.messages:
...
class HFTextGenerator(TextGenerator):
    def __init__(self, provider: str = "huggingface", device_map=None, **kwargs):
        ...
        self.dialogue_type = kwargs.get("dialogue_type", "codellama")

        self.model_name = kwargs.get("model", "codellama/CodeLlama-34b-Python-hf")
        ...
        if self.dialogue_type == "codellama":
            self.dialogue_template = DialogueTemplate(
                dialogue_type="codellama",
